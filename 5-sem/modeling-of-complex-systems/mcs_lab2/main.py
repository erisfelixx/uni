...
def build_operator_and_predict(
    ...
    _ = is_pseudoinverse(X_tilde, X_pinv)
...
def main():
    # === Вхідні зображення ===
    X = read_grayscale_image("x1.bmp")  # вхідний сигнал
    Y = read_grayscale_image("y5.bmp")  # вихідний сигнал

    # Важливо: однакова кількість стовпчиків (n) у X та Y
    if X.shape[1] != Y.shape[1]:
        raise ValueError(
            f"Кількість стовпчиків повинна збігатися, "
            f"а маємо: X {X.shape} vs Y {Y.shape}"
        )
